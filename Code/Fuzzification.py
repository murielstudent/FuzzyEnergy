...
def cluster(data, target_col, centroid_list, plot=True):
	...
	if str(centroid_list).isdigit():
		...
		print("Non valid input for centroid_list: should be integer or an array with length equals number of features")
		return(-1)
	collected = []
	# get target vector 
	Y = data[:, target_col]
	for i in range(feature_count):
		...
		# plot to see if satisfied and return clusters
